...
function getStatusColors(status: string, cssVars: Record<string, string>) {
  ...
  if (status === "done")        return { fill: yellow, stroke: yellow, text: cssVars["--primary-foreground"] || "#0a0a0a" }
  if (status === "in-progress") return { fill: green,  stroke: green,  text: cssVars["--primary-foreground"] || "#0a0a0a" }
...
export function RoadmapDiagram({ roadmap }: RoadmapDiagramProps) {
  ...
  const cssVars = useCssVars([
    "--neon-yellow", "--neon-green", "--neon-blue",
    "--card", "--border", "--foreground",
    "--primary-foreground", "--background", "--secondary",
  ])

  const handleNodeClick = useCallback((node: RoadmapNode) => { setSelectedNode(node) }, [])
        ...
        >
          <defs>
            ...
            <marker id="arrow-grey" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto">
              <path d="M0,0 L0,6 L8,3 z" fill={cssVars["--border"] || "#444444"} />
            </marker>
          </defs>
          ...
          {roadmap.edges.map((edge, i) => {
            ...
            const stroke = from.status === "done"
              ? yellow
              : from.status === "in-progress"
              ? green
              : cssVars["--border"] || "#333333"
            const markerId = from.status === "done"
              ? "arrow-yellow"
              : from.status === "in-progress"
              ? "arrow-green"
              : "arrow-grey"
          ...
          {roadmap.nodes.map((node) => {
            ...
            const { fill, stroke: baseStroke, text } = getStatusColors(node.status, cssVars)
            const isHovered = hoveredId === node.id
            const stroke = isHovered ? cssVars["--foreground"] || "#ffffff" : baseStroke
            ...
            for (const word of words) {
              if ((current + " " + word).trim().length > 22 && current.length > 0) {
                lines.push(current)
                current = word
              } else {
            ...
            const lineH = 16
            const textY = node.y + node.height / 2 - (lines.length * lineH) / 2 + lineH / 2
              ...
              >
                ...
                {(node.status !== "todo" || isHovered) && (
                  <rect
                    ...
                    rx={2} fill="none"
                    stroke={stroke} strokeWidth={1} opacity={0.25}
                  />
                )}
                ...
                <rect
                  ...
                  rx={2} fill={fill}
                  stroke={stroke} strokeWidth={isHovered ? 3 : strokeW}
                />
                ...
                {node.status === "in-progress" && (
                  <circle
                    cx={node.x + node.width - 10} cy={node.y + 10}
                    r={4}
